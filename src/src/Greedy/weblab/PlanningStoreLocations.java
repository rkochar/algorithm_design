...
public class PlanningStoreLocations {
	...
	public static Set<Store> donutTime(int n, int k, List<House> houses) {
		...
		UnionFindHouse unionFindHouse = new UnionFindHouse(houses);
		// TODO
		int size = n;
		distances.sort(Comparator.comparingLong(x -> x.distance));
		for (Distance distance: distances) {
			if (unionFindHouse.join(distance.a, distance.b)) size --;
			if (size <= k) break;
		}

		Set<Store> stores = new HashSet<>(k);
		for (List<House> housesList: unionFindHouse.clusters()) {
			int x = 0, y = 0, num = 0;
			...
			stores.add(new Store((double) x / num, (double) y / num));
		}
		return stores;
	}
}
...
class UnionFindHouse {
	...
	UnionFindHouse(List<House> houses) {
		this.houses = houses;
		int n = houses.size();
		parent = new int[n];
		rank = new int[n];
